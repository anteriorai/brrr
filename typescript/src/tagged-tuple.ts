...
export abstract class TaggedTuple {
  public static fromTuple<T extends TaggedTuple, A extends unknown[], Tag extends number>(this: Tagged<T, A, Tag>, tag: NoInfer<Tag>, ...tuple: A): T {
    if (tag !== this.tag) {
...
export abstract class TaggedTupleStrings extends TaggedTuple {
  ...
  public static decode<T extends TaggedTuple, A extends unknown[], Tag extends number>(this: Tagged<T, A, Tag>, data: Uint8Array): T {
    const decoded = bencoder.decode(data, TaggedTupleStrings.encoding) as [Tag, ...A];
